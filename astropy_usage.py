...
if len(sys.argv) > 1:
    ...
    step, queried_repos, gh_repo, gh_name, missed_repos = saved_results
else:
    step = 0
    queried_repos = 0
    gh_repo = set()
...
while queried_repos + missed_repos < total_repo:
    queried_repos_rollback = queried_repos
    ...
    if current_total > 1000:
        ...
        missed_repos += current_total - 1000
    ...
    try:
        ...
        print("Finished search up until step {} for a total of {} repos. "
              "Search failed after the {}th repo.".format(
                  step, queried_repos_rollback, queried_repos))
        queried_repos = queried_repos_rollback
        with open('usage_results.pkl', 'wb') as f:
            results = (step, queried_repos, gh_repo, gh_name, missed_repos)
